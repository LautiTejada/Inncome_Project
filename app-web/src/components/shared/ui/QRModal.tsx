...
import { FiX, FiCode, FiMapPin, FiHome, FiFileText } from 'react-icons/fi';
  ...
  useEffect(() => {
    if (qr) {
      setFormData({
        ...
        numero: qr.numero,
        localidad: qr.localidad,
        provincia: qr.provincia
      });
    } else {
      setFormData({
        nombre: '',
        ...
        <div className="flex items-start justify-between p-6 border-b border-[#0047bb]/20">
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-[#041e42]">
              {isEditing ? 'Editar QR' : 'Crear QR'}
            </h2>
            <p className="text-[#041e42] mt-1 font-medium">
              {isEditing ? 'Modifica la información del código QR' : 'Registra un nuevo código QR de forma rápida y sencilla'}
            </p>
          ...
          <div className="bg-white rounded-2xl border-2 border-[#0047bb]/40 shadow-md overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {['ESPECTADOR', 'BALNEARIO', 'TRABAJADOR', 'VISITA'].map(product => (
