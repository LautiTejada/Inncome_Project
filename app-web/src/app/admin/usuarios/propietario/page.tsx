...
import { FiShield, FiSearch, FiPlus, FiEdit, FiTrash2, FiEye } from 'react-icons/fi';
import { PageLayout, DataTable, TableHeader, TableRow, CreateUserModal, UserDetailsModal, UserEditModal } from '@/components';

export default function UsuariosPropietarioPage() {
  ...
  const [users, setUsers] = useState([
    {
      ...
      email: 'roberto.martinez@propietario.com',
      role: 'client',
      status: 'active',
      lastLogin: '2024-01-15 09:30',
      company: 'Propiedad ABC'
    },
    {
      id: '2',
      name: 'Silvia Fernández',
      email: 'silvia.fernandez@propietario.com',
      role: 'client',
      status: 'active',
      lastLogin: '2024-01-14 17:45',
      company: 'Propiedad XYZ'
    },
    {
      id: '3',
      name: 'Miguel Torres',
      email: 'miguel.torres@propietario.com',
      role: 'client',
      status: 'inactive',
      lastLogin: '2024-01-10 12:15',
      company: 'Propiedad 123'
    }
  ]);

  const handleExport = () => {
    console.log('Exportando usuarios propietarios...');
  };
  ...
  const handleCreateUser = (userData: { name: string; email: string; role: string; status: string; company?: string }) => {
    const newUser = {
      ...
      lastLogin: 'Nunca',
      company: userData.company || 'Sin propiedad'
    };
    setUsers(prev => [newUser, ...prev]);
    console.log('Usuario propietario creado:', newUser);
  };
  ...
  const handleSaveUser = (updatedUser: { id: string; name: string; email: string; role: string; status: string; lastLogin: string; company: string }) => {
    ...
    console.log('Usuario propietario actualizado:', updatedUser);
  };

  const handleDeleteUser = (userId: string) => {
    if (confirm('¿Está seguro de que desea eliminar este usuario propietario?')) {
      setUsers(prev => prev.filter(u => u.id !== userId));
      console.log('Usuario propietario eliminado:', userId);
    }
  };
  ...
  return (
    <PageLayout
      ...
      description="Gestión de usuarios propietarios de propiedades"
      showBackButton={true}
    ...
    >
      ...
      <div className="hidden lg:block">
        <DataTable title="Propietarios de Propiedades" showExport={true} onExport={handleExport}>
      ...
      <div className="lg:hidden space-y-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Propietarios de Propiedades</h3>
          <button 
        ...
        {users.map((user) => (
          <div key={user.id} className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-400">
              <div>
                <span className="font-medium">Propiedad:</span> {user.company}
              </div>
              <div>
