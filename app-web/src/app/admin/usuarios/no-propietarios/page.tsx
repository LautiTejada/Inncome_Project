...
import { FiEdit, FiSearch, FiPlus, FiTrash2, FiEye } from 'react-icons/fi';
import { PageLayout, DataTable, TableHeader, TableRow, CreateUserModal, UserDetailsModal, UserEditModal } from '@/components';

export default function UsuariosNoPropietariosPage() {
  ...
  const [users, setUsers] = useState([
    {
      id: '1',
      name: 'Fernando Ruiz',
      email: 'fernando.ruiz@usuario.com',
      role: 'client',
      status: 'active',
      lastLogin: '2024-01-15 13:30',
      company: 'Sin Propiedad'
    },
    {
      id: '2',
      name: 'Carmen Silva',
      email: 'carmen.silva@usuario.com',
      role: 'client',
      status: 'pending',
    ...
    },
    {
      id: '3',
      name: 'Ricardo Vargas',
      email: 'ricardo.vargas@usuario.com',
      role: 'client',
      status: 'inactive',
      lastLogin: '2024-01-11 10:15',
      company: 'Sin Propiedad'
    }
  ]);

  const handleExport = () => {
    console.log('Exportando usuarios no propietarios...');
  };
  ...
  const handleCreateUser = (userData: { name: string; email: string; role: string; status: string; company?: string }) => {
    const newUser = {
      ...
      lastLogin: 'Nunca',
      company: userData.company || 'Sin propiedad asignada'
    };
    setUsers(prev => [newUser, ...prev]);
    console.log('Usuario no propietario creado:', newUser);
  };
  ...
  const handleSaveUser = (updatedUser: { id: string; name: string; email: string; role: string; status: string; lastLogin: string; company: string }) => {
    ...
    console.log('Usuario no propietario actualizado:', updatedUser);
  };

  const handleDeleteUser = (userId: string) => {
    if (confirm('¿Está seguro de que desea eliminar este usuario no propietario?')) {
      setUsers(prev => prev.filter(u => u.id !== userId));
      console.log('Usuario no propietario eliminado:', userId);
    }
  };
  ...
  return (
    <PageLayout
      ...
      description="Gestión de usuarios sin propiedades asignadas"
      showBackButton={true}
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Buscar usuarios sin propiedades..."
      ...
      <div className="hidden lg:block">
        <DataTable title="Usuarios Sin Propiedades Asignadas" showExport={true} onExport={handleExport}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10">
                        <div className="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-cyan-600 flex items-center justify-center text-white font-semibold text-sm">
      ...
      <div className="lg:hidden space-y-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Usuarios Sin Propiedades Asignadas</h3>
          <button 
        ...
        {users.map((user) => (
          <div key={user.id} className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-400">
              <div>
                <span className="font-medium">Tipo:</span> {user.company}
              </div>
              <div>
